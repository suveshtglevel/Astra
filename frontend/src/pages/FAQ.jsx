...
const faqData = [
  { question: "How accurate is Klue, the AI astrologer?", answer: "Klue uses precise planetary ephemeris data combined with deep learning to interpret your chart accurately based on traditional astrological rules." },
...
export default function FAQ() {
  ...
  return (
    <div className="min-h-[85vh] py-24 px-6 md:px-12 max-w-3xl mx-auto relative z-10 pt-32">
      <h1 className="text-4xl md:text-5xl font-serif text-white mb-4 text-center">Frequently Asked Questions</h1>
      <p className="text-[#E7C36A] text-center mb-12 font-medium tracking-wide">Find answers to the most common questions about our platform and astrology.</p>

      <div className="space-y-4">
        {faqData.map((item, index) => (
          <div key={index} className="bg-white/5 border border-white/10 rounded-xl overflow-hidden transition-all duration-300 hover:border-[#E7C36A]/50">
            <button 
              className="w-full text-left px-6 py-5 flex justify-between items-center focus:outline-none"
              onClick={() => setOpenIndex(index === openIndex ? -1 : index)}
            >
              <h3 className="text-lg font-medium text-white">{item.question}</h3>
              {index === openIndex ? <ChevronUp className="text-[#E7C36A]" /> : <ChevronDown className="text-white/50" />}
            </button>
